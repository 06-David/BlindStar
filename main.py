...
class BlindStarApp:
    """BlindStar Application with Voice Integration"""

    def __init__(self, source=0, enable_voice=True, args=None):
        # 初始化核心视觉引擎
...
Examples:
  ...
  python main.py --source 0 --disable-voice    # Disable voice interaction
    ...
    def run_camera(self, args):
        """Run camera detection with optional voice support"""
        if args.disable_voice:
            self.logger.info("Starting camera detection (without voice)...")
            return self._run_camera_without_voice(args)
        else:
            self.logger.info("Starting camera detection with voice interaction...")
            return self._run_camera_with_voice(args)

    def _run_camera_with_voice(self, args):
        """Run camera with voice integration"""
        app = BlindStarApp(
    ...
    def _run_camera_without_voice(self, args):
        """Run camera without voice integration (original functionality)"""
