...
from config import ModelConfig, LogConfig, VideoConfig, PerformanceConfig, AmapConfig

# 导入新增的语音和POI模块
from core.tts_engine import TTSEngine
from core.stt_engine import STTEngine
from core.poi_query import POIQuery
from core.amap_navigation import AmapNavigation, VoiceNavigationHandler
...
class BlindStarApp:
    ...
    def __init__(self, source: int | str = 0, args: Optional[argparse.Namespace] = None):
        ...
        # 从参数中获取模块配置
        modules = getattr(args, 'module', ['vision', 'voice', 'distance', 'poi', 'tracking', 'analysis', 'navigation'])
        ...
        self.enable_navigation = 'navigation' in modules
    ...
    def initialize(self):
        ...
        if self.enable_voice:
            ...
            # 设置TTS和STT的互斥关系
            self.tts.set_stt_engine(self.stt)
        ...
        if self.enable_navigation:
            # 初始化导航模块
            self.navigation = AmapNavigation(
                api_key="717d9a827c0ac3521932d3ae59aebbfe"
            )
            if self.poi:
                self.navigation.set_poi_module(self.poi)

            # 初始化语音导航处理器
            if self.enable_voice and self.tts:
                self.voice_nav_handler = VoiceNavigationHandler(self.navigation, self.tts)
                if self.poi:
                    self.voice_nav_handler.set_poi_module(self.poi)

            logging.info("导航模块初始化完成")

        # 初始提示
        if self.enable_voice and self.tts:
            if self.enable_navigation:
                self.tts.speak("系统已启动，支持POI查询和语音导航功能", blocking=True)
            else:
    ...
    def handle_voice_command(self, command: str):
        ...
        # 优先检查导航命令
        if self.enable_navigation and self.voice_nav_handler:
            if self.voice_nav_handler.handle_command(command):
                return  # 导航命令已处理


        # 命令路由
        if "附近" in command or "查找" in command or "搜索" in command:
            ...
            # 查找匹配的关键词
            keyword = None
            for kw, mapped_kw in keywords_map.items():
                if kw in command:
                    keyword = mapped_kw
                    break
            ...
            if keyword:
                ...
                # 没有识别到具体关键词，提示用户
                available_types = "餐厅、咖啡厅、超市、银行、医院、火车站、地铁站、公交站、公园、酒店、厕所"
                self.tts.speak(f"请说明要搜索什么，例如：{available_types}等")
...
使用示例:
  ...
  python main.py --module voice poi navigation # 启用语音、POI查询和导航
...
注意: 所有处理结果自动保存到logs文件夹中
        ...
        parser.add_argument(
            '--module',
            nargs='+',
            choices=['vision', 'voice', 'distance', 'poi', 'tracking', 'analysis', 'navigation'],
            default=['vision', 'voice', 'distance', 'poi', 'tracking', 'analysis', 'navigation'],
