...
import time
from tqdm import tqdm  # 引入tqdm用于进度条统一显示

from .frame_analyzer import FrameAnalyzer, VideoAnalysisStats
...
class BatchProcessingConfig:
    ...
    # 深度模型输入尺寸配置
    input_size: Tuple[int, int] = (384, 512)  # (width, height)
...
class BatchImageProcessor:
    ...
    def process_directory(self, 
        ...
        # 限制处理图像数量
        if self.config.max_images and self.config.max_images > 0:
            original_count = len(image_files)
            image_files = image_files[:self.config.max_images]
            logger.info(f"限制处理图像数量: {original_count} -> {len(image_files)} (max_images={self.config.max_images})")
        
        # 初始化BlindStar
        from . import BlindStar
        from .depth_visualizer import DepthVisualizer
        ...
        depth_visualizer = DepthVisualizer(
            model_type=self.config.midas_model,
            device=self.config.device
        )
            ...
            for i, image_file in enumerate(image_files, 1):
                ...
                try:
                    ...
                    if save_results:
                        ...
                        depth_dir = Path(self.config.output_base_dir) / "depth"
                        output_dir.mkdir(parents=True, exist_ok=True)
                        depth_dir.mkdir(parents=True, exist_ok=True)
                        output_path = str(output_dir / f"detected_{image_file.stem}.jpg")
                        depth_output_path = str(depth_dir / f"depth_{image_file.stem}.jpg")
                    ...
                    # 生成并保存深度图
                    if save_results and depth_output_path:
                        depth_color = depth_visualizer.get_colormap(image)
