                ...
                if frame_count % self.frame_skip == 0:
                    ...
                    if detections:
                        for i, det in enumerate(detections):
                            logger.info(f"  Detection {i+1}: {det.class_name} (confidence: {det.confidence:.2f})")
...
def process_video_async(processor: VideoProcessor,
                       input_path: str,
                       output_path: str,
                       callback: Optional[Callable[[VideoProcessingStats], None]] = None,
    ...
    def _process():
        try:
            stats = processor.process_video_file(input_path, output_path, max_duration)
            if callback:
                callback(stats)
        except Exception as e:
