...
class FrameAnalyzer:
    ...
    def __init__(self, 
        ...
        if self.enable_detailed_log:
            log_file = self.log_dir / f"detailed_analysis_{self.session_id}.log"
        ...
        self.logger.info(f"Frame analyzer initialized for session {self.session_id}")
    ...
    def analyze_frame(self, 
        ...
        # 记录详细日志
        if self.enable_detailed_log:
            self.logger.info(f"Frame {frame_number}: {len(objects)} objects detected, "
                           f"processing time: {processing_time_ms:.2f}ms")
            
            for obj in objects:
                log_msg = (f"  Object: {obj.class_name} (conf={obj.confidence:.2f}) "
                          f"at ({obj.center_x:.0f},{obj.center_y:.0f})")
                
                if obj.distance_meters:
                    log_msg += f", distance={obj.distance_meters:.2f}m"
                
                if obj.speed_mps:
                    log_msg += f", speed={obj.speed_mps:.2f}m/s ({obj.speed_kmh:.1f}km/h)"
